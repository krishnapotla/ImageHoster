...
public class ImageRepository {
    ...
    public Image getImageByTitle(int id, String title) {
        EntityManager em = emf.createEntityManager();
        try {
            TypedQuery<Image> typedQuery = em.createQuery("SELECT i from Image i where i.id=:id and i.title =:title", Image.class).setParameter("title", title).setParameter("id", id);
